...
from sklearn.ensemble import RandomForestClassifier # Specific model for this file (RandomForestClassifier)
...
# --- Specific Tool Functions for Random Forest Classification ---

def build_random_forest_pipeline(ml_state: MLWorkflowState,
    ...
    """
    Tool to build a scikit-learn preprocessing and Random Forest Classifier model pipeline.
    ...
    # Random Forests are not sensitive to feature scaling, so 'none' is often suitable.
    if numeric_scaler_params["type"] == "standard":
        ...
        numeric_scaler_obj = 'passthrough' # Default for tree-based models
    else:
    ...
    # Instantiate the Random Forest Classifier model with specified parameters
    ...
    print(f"Pipeline with Random Forest Classifier built and stored with key: {ml_state.pipeline_key}")
    print(f"Model Parameters: {model_params}")


# --- Orchestrator Tool: run_random_forest_workflow ---
def run_random_forest_classification_workflow(
    path: str,
    target_column: str,
    ...
    rf_criterion: str = "gini", # Common criteria for classification: 'gini' or 'entropy'
    rf_max_depth: Optional[int] = None,
    rf_min_samples_split: int = 2,
    rf_min_samples_leaf: int = 1,
    rf_max_features: str = "sqrt", # 'sqrt' is the default for classification in scikit-learn
    rf_n_jobs: int = -1, # Use all available cores
) -> Dict[str, Any]: # Return a dictionary for the agent's observable output
    """
    A comprehensive tool to orchestrate an end-to-end Random Forest Classifier workflow.
    """
    ...
    print(f"Starting Random Forest Classification workflow for {path} with target '{target_column}'...")
    ...
    try:
        ...
        print("\n--- Skipping Correlation Drop (not typically necessary for Random Forests) ---")
        ...
        build_random_forest_pipeline( # Call the specific pipeline builder
            ml_state,
        ...
        train_model(ml_state, apply_log_transform=False) # No log transform for RF classification target
