...
from sklearn.svm import SVR # Specific model for this file (SVR)
...
# --- Specific Tool Functions for SVR Regression ---

def build_svr_pipeline(ml_state: MLWorkflowState,
    ...
    """
    Tool to build a scikit-learn preprocessing and Support Vector Regressor (SVR) model pipeline.
    ...
    # SVR is sensitive to feature scaling, so 'none' is generally not recommended here.
    if numeric_scaler_params["type"] == "standard":
        ...
        numeric_scaler_obj = 'passthrough' # Allow 'none' but it's often not optimal for SVR
    else:
    ...
    # Instantiate the Support Vector Regressor model with specified parameters
    ...
    print(f"Pipeline with SVR built and stored with key: {ml_state.pipeline_key}")
    print(f"Model Parameters: {model_params}")


# --- Orchestrator Tool: run_svr_workflow ---
def run_svr_workflow(
    ...
    numeric_scaler_type: str = "standard", # Default to 'standard' for SVR as it's scale-sensitive
    ...
    """
    A comprehensive tool to orchestrate an end-to-end Support Vector Regressor (SVR) workflow.
    """
    ...
    print(f"Starting SVR workflow for {path} with target '{target_column}'...")
    ...
    try:
        ...
        # NOTE: Correlation drop step is typically skipped for SVR.
        print("\n--- Skipping Correlation Drop (not typically necessary for SVR) ---")
        ...
        build_svr_pipeline( # Call the specific pipeline builder
            ml_state,
        ...
        train_model(ml_state, apply_log_transform=False) # No log transform for SVR by default
        ml_state.workflow_summary["steps_completed"].append("Model Trained")

        # Step 8: Evaluate Model
        print("\n--- Step 8: Evaluating Model ---")
        evaluate_model(ml_state, apply_exp_transform=False) # No exp transform for SVR by default
