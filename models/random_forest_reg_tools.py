...
from sklearn.ensemble import RandomForestRegressor # Specific model for this file
...
# --- Specific Tool Functions for Random Forest Regression ---
...
def build_random_forest_pipeline(ml_state: MLWorkflowState,
    ...
    """
    Tool to build a scikit-learn preprocessing and Random Forest Regressor model pipeline.
    ...
    random_forest_model = RandomForestRegressor(**model_params)
    ...
    print(f"Pipeline with RandomForestRegressor built and stored with key: {ml_state.pipeline_key}")
...
def run_random_forest_workflow(
    ...
    rf_max_features: str = "sqrt", # 'sqrt' is the default for regression in scikit-learn
    rf_n_jobs: int = -1, # Use all available cores
) -> Dict[str, Any]: # Return a dictionary for the agent's observable output
    """
    A comprehensive tool to orchestrate an end-to-end Random Forest Regressor workflow.
    """
    ...
    print(f"Starting Random Forest workflow for {path} with target '{target_column}'...")
    ...
    try:
        ...
        train_model(ml_state, apply_log_transform=False) # No log transform for RF by default
        ml_state.workflow_summary["steps_completed"].append("Model Trained")

        # Step 8: Evaluate Model
        print("\n--- Step 8: Evaluating Model ---")
        evaluate_model(ml_state, apply_exp_transform=False) # No exp transform for RF by default
