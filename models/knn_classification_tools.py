...
from sklearn.neighbors import KNeighborsClassifier # Specific model for this file (KNeighborsClassifier)
...
# --- Specific Tool Functions for KNN Classification ---

def build_knn_pipeline(ml_state: MLWorkflowState,
    ...
    """
    Tool to build a scikit-learn preprocessing and K-Nearest Neighbors Classifier model pipeline.
    ...
    # KNN is sensitive to feature scaling, so 'none' is generally not recommended here.
    if numeric_scaler_params["type"] == "standard":
        ...
        numeric_scaler_obj = 'passthrough' # Allow 'none' but it's often not optimal for KNN
    else:
    ...
    # Instantiate the K-Nearest Neighbors Classifier model with specified parameters
    ...
    print(f"Pipeline with K-Nearest Neighbors Classifier built and stored with key: {ml_state.pipeline_key}")
    print(f"Model Parameters: {model_params}")


# --- Orchestrator Tool: run_knn_workflow ---
def run_knn_workflow(
    ...
    numeric_scaler_type: str = "standard", # Default to 'standard' for KNN as it's scale-sensitive
    ...
    knn_p: int = 2, # Power parameter for the Minkowski metric
) -> Dict[str, Any]: # Return a dictionary for the agent's observable output
    """
    A comprehensive tool to orchestrate an end-to-end K-Nearest Neighbors Classifier workflow.
    """
    ...
    print(f"Starting K-Nearest Neighbors Classification workflow for {path} with target '{target_column}'...")
    ...
    try:
        ...
        # NOTE: Correlation drop step is typically skipped for KNN.
        print("\n--- Skipping Correlation Drop (not typically necessary for KNN) ---")
        ...
        build_knn_pipeline( # Call the specific pipeline builder
            ml_state,
        ...
        train_model(ml_state, apply_log_transform=False) # No log transform for KNN classification target
