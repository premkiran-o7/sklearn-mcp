...
from sklearn.naive_bayes import GaussianNB # Specific model for this file (Gaussian Naive Bayes)
...
from shared_tools import (
    ...
    select_scaler, evaluate_classification_model, _engineer_features,
    llm, FeatureEngineeringPrompt, train_model
)

# --- Specific Tool Functions for Naive Bayes Classification ---

def build_naive_bayes_pipeline(ml_state: MLWorkflowState,
    ...
    """
    Tool to build a scikit-learn preprocessing and Gaussian Naive Bayes Classifier model pipeline.
    ...
    # Naive Bayes is generally not sensitive to feature scaling, but some variants might benefit.
    # GaussianNB assumes Gaussian distribution, which can be affected by scale.
    ...
    # Instantiate the Gaussian Naive Bayes Classifier model with specified parameters
    nb_model = GaussianNB(**model_params)
    ...
    print(f"Pipeline with Gaussian Naive Bayes Classifier built and stored with key: {ml_state.pipeline_key}")
    print(f"Model Parameters: {model_params}")


# --- Orchestrator Tool: run_naive_bayes_workflow ---
def run_naive_bayes_workflow(
    ...
    numeric_scaler_type: str = "standard", # Default to 'standard' for GaussianNB
    categorical_encoder_type: str = "onehot",
    # --- Gaussian Naive Bayes Classifier Hyperparameters ---
    nb_var_smoothing: float = 1e-9, # Portion of the largest variance of all features that is added to variances for calculation stability
) -> Dict[str, Any]: # Return a dictionary for the agent's observable output
    """
    A comprehensive tool to orchestrate an end-to-end Gaussian Naive Bayes Classifier workflow.
    """
    ...
    print(f"Starting Gaussian Naive Bayes Classification workflow for {path} with target '{target_column}'...")
    ...
    try:
        ...
        # NOTE: Correlation drop step is typically skipped for Naive Bayes.
        print("\n--- Skipping Correlation Drop (not typically necessary for Naive Bayes) ---")
        ...
        build_naive_bayes_pipeline( # Call the specific pipeline builder
            ml_state,
        ...
        train_model(ml_state, apply_log_transform=False) # No log transform for Naive Bayes target
        ml_state.workflow_summary["steps_completed"].append("Model Trained")

        # Step 8: Evaluate Model
        print("\n--- Step 8: Evaluating Model ---")
        evaluate_classification_model(ml_state) # Use classification evaluation
